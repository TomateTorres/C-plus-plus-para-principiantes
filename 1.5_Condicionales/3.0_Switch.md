...
La etiqueta `default` es opcional y sólo puede haber una por cada declaración `switch` (podemos tener tantos `case` como queramos pero sólo un `default`). Por convención, la etiqueta `default` va al final de la declaración `switch`.
...
En los ejemplos anteriores, hemos estado usando declaraciones `return` para detener la ejecución de instrucciones después de nuestras etiquetas. Sin embargo, [como ya vimos en la sección sobre funciones](../1.3_Funciones_basicas/0_Introducción.md), al usar `return` salimos de la función y volvemos al llamador de ésta.
...
Lo mejor es utilizar `break` en vez de `return` a menos que deliberadamente queramos salir de la función y volver a su llamador.
