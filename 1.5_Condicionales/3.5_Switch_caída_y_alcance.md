...
default: // no se crea un bloque implícito
    std::cout << "Caso default \n"; // esto es parte del alcance de `switch`, no un bloque implícito de `case 2`
    break; // esto es parte del alcance de `switch`, no un bloque implícito de `case 2`
}
```
...
```
En el código anterior, el error vino de haber definido la misma variable `x` dentro del mismo alcance (en este caso `switch`).
...
```
El código anterior no compilará si nuestra configuración de compilador es restrictiva (si trata advertencias como errores) y al usarlo en una configuración más restrictiva sí lo hará pero nunca imprimirá `=＾● ⋏ ●＾=`.
