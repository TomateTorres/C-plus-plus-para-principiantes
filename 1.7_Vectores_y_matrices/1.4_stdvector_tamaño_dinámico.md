...
int main()
{
  ...
  /*
  Un "número feliz" es un número entero positivo que, al aplicar repetidamente el proceso de reemplazar el número por la suma de los cuadrados de sus dígitos, eventualmente llega a 1.
  */
  ...
  // Primero, usaremos `.push_back()` para aumentar en uno la longitud del vector (necesitamos)
  int ultimaPos{ numerosFelicesDosCifras.size() -1 };
  numerosFelicesDosCifras.push_back(numerosFelicesDosCifras[ultimaPos]);
