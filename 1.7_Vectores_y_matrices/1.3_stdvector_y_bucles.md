...
Siguiendo la misma lógica que teníamos para [pasar `std::vector`*es* como argumentos de funciones](1.2_stdvector_como_argumento_de_una_función.md), dado un array `v` que usaremos dentro de un ciclo `for` basado en rango, tenemos varias formas de pasar dicho vector:
